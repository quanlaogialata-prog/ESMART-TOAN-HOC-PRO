...
const regex = /if \(newFile\) \{\s*testData\.fileUrl = finalFileUrl \|\| "";\s*testData\.fileName = newFile\.name;\s*if \(extractionFileUrl\) \{\s*setSysMsg\('Đang dùng AI trích xuất câu hỏi từ tài liệu\.\.\. Vui lòng chờ\.\.\.'\);[\s\S]*?\} catch\(e\) \{\s*console\.error\("Extraction error", e\);\s*setSysError\('Lỗi kết nối khi trích xuất tài liệu\.'\);\s*setIsSaving\(false\);\s*return;\s*\}\s*\}\s*\}/;

const newBlock = `if (newFile) {
