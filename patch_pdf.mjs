...
// 1. Add jsPDF imports if missing
...
const targetOld = `  const exportGradebookForAssignment = (assignmentId: string, className: string, testTitle: string) => {
...
const replacementNew = `  const removeVietnameseTones = (str: string) => {
    if (!str) return '';
...
// Replace the function call onClick={() => exportGradebookForAssignment(a.id, a.className, a.testTitle)}
content = content.replace(
  `exportGradebookForAssignment(a.id, a.className, a.testTitle)`,
  `exportGradebookForAssignment(a)`
);
