...
const clearCanvas = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.error('Lỗi khi lưu bảng vẽ.');
    } finally {
      setIsSaving(false);
    }
  };
      ...
      <div className={`bg-[#f8f9fa] shadow-2xl flex flex-col overflow-hidden border border-gray-200 transition-all duration-300 ${isFullscreen ? 'w-full h-full rounded-none' : 'w-full max-w-5xl h-[90vh] rounded-2xl'}`}>
        ...
        <div className="px-4 py-2 border-b bg-white/50 flex flex-wrap gap-4 items-center shrink-0 shadow-sm z-20 relative">
          ...
          {(tool === 'math' || elements.find(e => e.id === editingElementId)?.type === 'math') && (
            <div className="flex gap-1 items-center bg-white p-1.5 rounded-xl shadow-sm border border-gray-100 text-sm font-serif overflow-x-auto max-w-[400px]">
              <button onClick={() => { const mf = document.querySelector('math-field') as any; if (mf) { mf.insert('\\sqrt{}'); mf.executeCommand('moveToPreviousChar'); mf.focus(); } }} className="px-2 py-1 hover:bg-gray-100 rounded font-bold whitespace-nowrap" title="Căn bậc hai">√x</button>
              <button onClick={() => { const mf = document.querySelector('math-field') as any; if (mf) { mf.insert('^{}'); mf.executeCommand('moveToPreviousChar'); mf.focus(); } }} className="px-2 py-1 hover:bg-gray-100 rounded font-bold whitespace-nowrap" title="Lũy thừa">x²</button>
              <button onClick={() => { const mf = document.querySelector('math-field') as any; if (mf) { mf.insert('\\left| \\right|'); mf.executeCommand('moveToPreviousChar'); mf.focus(); } }} className="px-2 py-1 hover:bg-gray-100 rounded font-bold whitespace-nowrap" title="Giá trị tuyệt đối">|x|</button>
              <button onClick={() => { const mf = document.querySelector('math-field') as any; if (mf) { mf.insert('\\frac{}{}'); mf.executeCommand('moveToPreviousChar'); mf.executeCommand('moveToPreviousChar'); mf.focus(); } }} className="px-2 py-1 hover:bg-gray-100 rounded font-bold whitespace-nowrap" title="Phân số">a/b</button>
